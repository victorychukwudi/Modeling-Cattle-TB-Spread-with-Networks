...
#   01. Libraries
#   02. Load & filter data
#   03. Variable engineering
#   04. Clustering variables, log transform, scale
#   05. Descriptive table
#   06. Exploration plots (histograms, density, scatter,
#       correlation, boxplots, severity vs spread scatter)
#   07. PCA
#   08. K-means on PCA space (exploratory)
#   09. Cluster tendency (Hopkins, VAT)
#   10. Optimal K selection (WSS, ClusterR elbow, silhouette,
#       gap stat, NbClust)
#   11. Hierarchical clustering (exploratory)
#   12. Canonical PAM K=3
#   13. PAM K comparison (K=2 to 5) with visuals
#   14. PAM silhouette comparison
#   15. K-means models (K=2 to 5) with means, visuals,
#       silhouette
#   16. K-means vs PAM silhouette comparison
#   17. Clusterboot stability
#   18. Cluster proportion by year
#   19. Survival analysis 1: BD duration
#   20. Survival analysis 2: Time to next BD
#   21. Internal multinomial + marginal effect barplots
#   22. Main risk factor multinomial
#   23. Year interaction model
#   24. Model fit, predicted probs, marginal effects plot
############################################################
...
bd_victory <- bd_victory %>%
  mutate(
    ...
    herd_size_mean = rowMeans(
      select(., mean_herd_size_jan, mean_herd_size_may, mean_herd_size_sep)
    )
  )

############################################################
# 03c. HISTORY VARIABLE
############################################################
...
bd_victory <- bd_victory %>%
  ...
  mutate(
    no_bds_started_in_prev_5yr = sapply(
      bd_start,
      function(x) sum(bd_start < x & bd_start >= x %m-% years(5), na.rm = TRUE)
    )
  ) %>%
  ungroup()

############################################################
# 03d. CORE EPIDEMIOLOGY
############################################################

bd_victory <- bd_victory %>%
  mutate(
    total_index_reactors =
      cows_positive_index_test + bulls_positive_index_test +
      steers_positive_index_test + heifers_positive_index_test +
    ...
    total_subsequent_reactors =
      (cows_positive + bulls_positive + steers_positive +
         heifers_positive + calves_positive) - total_index_reactors,
    prop_positive_index = ifelse(
      total_tested_index > 0, total_index_reactors / total_tested_index, NA),
    spread_ratio = ifelse(
      total_index_reactors > 0,
      total_subsequent_reactors / total_index_reactors, NA),
    spread_any = ifelse(total_subsequent_reactors > 0, 1, 0)
  ) %>%
  mutate(spread_ratio = ifelse(is.infinite(spread_ratio), NA, spread_ratio))

############################################################
# 04. CLUSTERING VARIABLES, LOG TRANSFORM, SCALE
#
# Five variables retained:
#   - spread_any REMOVED: binary duplicate of spread_ratio
#   - bd_within_13_months REMOVED: derived from survival outcome
############################################################

cluster_vars <- c(
  "prop_positive_index", "spread_ratio", "bd_duration_days",
...
cluster_vars_log <- c(
  "prop_positive_index", "spread_ratio_log", "bd_duration_days_log",
...
desc_table <- bd_core %>%
  ...
  tidyr::pivot_longer(cols=everything(),
                      names_to=c("Variable",".value"), names_pattern="(.*)_(Mean|SD|Min|Max)")

print(desc_table)
write.csv(desc_table, "00_descriptive_table.csv", row.names = FALSE)

############################################################
# 06. EXPLORATION PLOTS
############################################################

png("01a_hist_raw.png", 1600, 1200)
bd_core %>% select(all_of(cluster_vars)) %>% pivot_longer(cols=everything()) %>%
...
bd_core %>% select(all_of(cluster_vars_log)) %>% pivot_longer(cols=everything()) %>%
  ggplot(aes(value)) + geom_histogram(bins=40, fill="darkgreen") +
  facet_wrap(~name, scales="free") + theme_bw() +
  ggtitle("Log-transformed distributions")
dev.off()

png("01c_density.png", 1600, 1200)
bd_core %>% select(all_of(cluster_vars_log)) %>% pivot_longer(cols=everything()) %>%
  ggplot(aes(value)) + geom_density(fill="orange", alpha=0.5) +
  facet_wrap(~name, scales="free") + theme_bw()
dev.off()
...
corrplot(cor(bd_scaled_full, use="pairwise.complete.obs"),
         method="color", type="upper", order="hclust",
         addCoef.col="black", number.cex=0.5, tl.cex=0.6)
dev.off()

png("01g_boxplots.png", 1600, 1200)
bd_scaled_full %>% pivot_longer(cols=everything()) %>%
  ggplot(aes(x=name, y=value)) + geom_boxplot(fill="grey") + theme_bw() +
...
dev.off()
############################################################
# POST-CLUSTER SCATTER MATRIX — coloured by PAM cluster
############################################################

cluster_colours <- c("1" = "#E41A1C",   # red
                     "2" = "#377EB8",   # blue
                     "3" = "#4DAF4A")   # green

plot_data <- bd_scaled_full %>%
  mutate(cluster = bd_core$cluster)

png("post_cluster_scatter_matrix.png", 1800, 1600)
pairs(
  plot_data[, cluster_vars_log],
  col     = cluster_colours[as.character(plot_data$cluster)],
  pch     = 19,
  cex     = 0.3,
  ...
  main    = "Scatter matrix of clustering variables coloured by PAM cluster (K = 3)"
)
...
fviz_pca_ind(bd_pca, geom="point", pointsize=0.8, alpha.ind=0.3,
...
############################################################
# HISTOGRAM – Distribution of variables after log
# transformation and scaling (PCA input matrix)
############################################################

png("01h_hist_scaled.png", 1600, 1200)
bd_scaled_full %>%
  ...
  theme_bw() +
  labs(
    title = "Distribution of clustering variables after log-transformation and standardisation",
    x     = "Standardised value",
    y     = "Count"
  )
dev.off()
############################################################
# PCA VARIABLE CONTRIBUTION – PC3
############################################################

png("04d2_var_contrib_PC3.png", 1200, 900)
fviz_contrib(bd_pca, choice = "var", axes = 3)
dev.off()
png("04e_pca_biplot.png", 1200, 900)
autoplot(bd_pca, loadings=TRUE, loadings.colour="darkblue",
...
# ClusterR elbow (variance explained)
png("02b2_clusterR_elbow.png", 1200, 900)
set.seed(123)
opt <- Optimal_Clusters_KMeans(nb_sample, max_clusters=10,
                               plot_clusters=TRUE, verbose=FALSE)
dev.off()
...
set.seed(123)
nb_all <- NbClust(data=nb_sample, distance="euclidean",
...
fviz_dend(hc, k=3, rect=TRUE, show_labels=FALSE,
          main="Hierarchical Clustering - Ward's Method (n=3,000)")
dev.off()
...
write.csv(as.data.frame(pam3_canonical$medoids), "pam_k3_medoids_real.csv", row.names=FALSE)
...
write.csv(pam3_means_raw, "pam_k3_cluster_raw_means.csv", row.names=FALSE)

############################################################
# 13. PAM K COMPARISON (K=2 to 5)
############################################################
...
gridExtra::grid.arrange(p_pam2, p_pam3, p_pam4, ncol=2)
dev.off()

png("03bm_pam_k3_plot.png", 1200, 900)
print(p_pam3)
dev.off()


p_pca2 <- fviz_cluster(
  list(data = vis_data, cluster = pam2$clustering[vis_idx]),
  geom = "point", ellipse.type = "norm"
) + ggtitle("PAM K=2 on PCA")

p_pca3 <- fviz_cluster(
  list(data = vis_data, cluster = pam3_canonical$clustering[vis_idx]),
  geom = "point", ellipse.type = "norm"
) + ggtitle("PAM K=3 on PCA")

p_pca4 <- fviz_cluster(
  list(data = vis_data, cluster = pam4$clustering[vis_idx]),
  geom = "point", ellipse.type = "norm"
) + ggtitle("PAM K=4 on PCA")

p_pca5 <- fviz_cluster(
  list(data = vis_data, cluster = pam5$clustering[vis_idx]),
  geom = "point", ellipse.type = "norm"
) + ggtitle("PAM K=5 on PCA")

png("03c_pca_pam_clusters.png", 1600, 1200)
gridExtra::grid.arrange(p_pca2, p_pca3, p_pca4, ncol = 2)
dev.off()
...
grid.arrange(sp2, sp3, sp4, ncol=2)
dev.off()
...
############################################################
# 15. K-MEANS MODELS (K=2 to 5) — comparator method
############################################################
...
k2_means <- km_df %>% mutate(cluster=bd_clusters2$cluster) %>%
  group_by(cluster) %>% summarise(across(everything(), mean, na.rm=TRUE), .groups="drop")
k3_means <- km_df %>% mutate(cluster=bd_clusters3$cluster) %>%
  group_by(cluster) %>% summarise(across(everything(), mean, na.rm=TRUE), .groups="drop")
k4_means <- km_df %>% mutate(cluster=bd_clusters4$cluster) %>%
  group_by(cluster) %>% summarise(across(everything(), mean, na.rm=TRUE), .groups="drop")
k5_means <- km_df %>% mutate(cluster=bd_clusters5$cluster) %>%
  group_by(cluster) %>% summarise(across(everything(), mean, na.rm=TRUE), .groups="drop")
...
# Cluster visuals (reuse vis_idx and vis_data defined in section 13)
km_p2 <- fviz_cluster(list(data=vis_data, cluster=bd_clusters2$cluster[vis_idx]),
                      geom="point", ellipse.type="norm") + ggtitle("K-means K=2")
km_p3 <- fviz_cluster(list(data=vis_data, cluster=bd_clusters3$cluster[vis_idx]),
                      geom="point", ellipse.type="norm") + ggtitle("K-means K=3")
km_p4 <- fviz_cluster(list(data=vis_data, cluster=bd_clusters4$cluster[vis_idx]),
                      geom="point", ellipse.type="norm") + ggtitle("K-means K=4")
km_p5 <- fviz_cluster(list(data=vis_data, cluster=bd_clusters5$cluster[vis_idx]),
                      geom="point", ellipse.type="norm") + ggtitle("K-means K=5")

png("kmeans_cluster_plots.png", 1600, 1200)
gridExtra::grid.arrange(km_p2, km_p3, km_p4, ncol=2)
dev.off()

# K-means silhouette (reuse sil_idx and dist_sil defined in section 14)
...
grid.arrange(ks2, ks3, ks4, ncol=2)
dev.off()
...
sil_comparison <- data.frame(
  Method = c(rep("K-means",3), rep("PAM",3)),
  K      = rep(c(2,3,4), 2),
  Avg_Silhouette = c(
    avg_sil(km_sil2), avg_sil(km_sil3), avg_sil(km_sil4),
    avg_sil(pam_sil2), avg_sil(pam_sil3), avg_sil(pam_sil4)
  )
)
...
ggplot(sil_comparison, aes(x=factor(K), y=Avg_Silhouette, fill=Method)) +
  ...
  labs(title="K-means vs PAM Silhouette Comparison",
       x="Number of Clusters (K)", y="Average Silhouette Width")
dev.off()


############################################################
# GAUSSIAN MIXTURE MODEL (GMM) — exploratory comparator
# mclust selects optimal model and number of components
# via BIC. Used as a sensitivity check alongside PAM.
############################################################

library(mclust)

set.seed(123)
gmm_fit <- Mclust(bd_scaled_full)

cat("\nGMM optimal model:\n")
print(summary(gmm_fit))

cat("\nGMM optimal number of components:", gmm_fit$G, "\n")
cat("GMM model type:", gmm_fit$modelName, "\n")

# BIC plot
png("04a_mclust_BIC.png", 1200, 900)
plot(gmm_fit, what = "BIC",
     main = "GMM Model Selection — BIC across components and covariance structures")
dev.off()

# Classification plot in PCA space
png("04b_mclust_classification.png", 1200, 900)
plot(gmm_fit, what = "classification",
     main = paste0("GMM Classification (K = ", gmm_fit$G, 
                   ", Model = ", gmm_fit$modelName, ")"))
dev.off()

# Uncertainty plot
png("04c_mclust_uncertainty.png", 1200, 900)
plot(gmm_fit, what = "uncertainty",
     main = "GMM Classification Uncertainty")
dev.off()

# Save GMM cluster assignments
bd_core$gmm_cluster <- gmm_fit$classification

# Compare GMM vs PAM using ARI
gmm_ari <- mclust::adjustedRandIndex(
  pam3_canonical$clustering,
  gmm_fit$classification
)
cat(sprintf("\nARI between PAM K=3 and GMM (K=%d): %.4f\n",
            gmm_fit$G, gmm_ari))
...
gmm_summary <- data.frame(
  Metric = c("Optimal K", "Model type", "BIC",
             "ARI vs PAM K=3"),
  Value  = c(gmm_fit$G, gmm_fit$modelName,
             round(gmm_fit$bic, 2),
             round(gmm_ari, 4))
)
write.csv(gmm_summary, "gmm_summary.csv", row.names = FALSE)
...
nd_rec <- expand.grid(
  ...
  bd_duration_days_log = quantile(mn_internal_data$bd_duration_days_log,
...
barplot(t(probs_rec), beside=TRUE, legend=TRUE,
        names.arg=c("No recur/Short","No recur/Long","Recur/Short","Recur/Long"),
        ylab="Predicted probability", col=2:4,
        main="Effect of recurrence and duration on cluster membership", las=2)
dev.off()
...
model_data <- final_data %>%
  select(cluster, Control_moves, NewBD_moves, Burnout_moves, Unknown_moves,
         Burnout_neighbours, Control_neighbours, NewBD_neighbours) %>%
  mutate(across(c(Burnout_neighbours, Control_neighbours, NewBD_neighbours),
                ~replace_na(.,0))) %>%
  na.omit()
...
model_data_yr <- final_data %>%
  filter(cluster != "0") %>%
  select(cluster, bd_start_yr, Control_moves, NewBD_moves, Burnout_moves,
         Unknown_moves, Burnout_neighbours, Control_neighbours, NewBD_neighbours) %>%
  mutate(across(c(Burnout_neighbours, Control_neighbours, NewBD_neighbours),
                ~replace_na(.,0))) %>%
  na.omit()
...
cat("\nmodel_data_yr cluster table:\n")
print(table(model_data_yr$cluster))

multi_model_yr <- multinom(
  cluster ~ NewBD_neighbours * bd_start_yr + Burnout_neighbours * bd_start_yr +
    Control_moves + NewBD_moves + Burnout_moves + Unknown_moves + Control_neighbours,
...
############################################################
# 24. PREDICTED PROBABILITIES & MARGINAL EFFECTS
############################################################
...
plot_long <- rf_effects %>%
  pivot_longer(cols=c("Control","Cluster 1","Cluster 2","Cluster 3"),
...
############################################################
# bTB BREAKDOWN CLUSTERING – ANDREW & JAMIE FEEDBACK ADDITIONS (v3, FULLY CHECKED)
#
# Run AFTER btb_main_pipeline_v3.R in the SAME R session.
# Required objects (created by the main script):
#   bd_scaled_full, bd_core, cluster_vars_log, pam3_canonical,
#   model_data, multi_model, null_model, data_bd, multi_data,
#   control_herds
#
# KEY FIX vs PREVIOUS DRAFT:
#   Section D had the SAME factor/numeric bind_rows() bug as the
#   main script originally had (controls_s$cluster <- 0, numeric).
#   This is now fixed using the same character-first pattern.
#
# Sections:
#   A. Cluster stability – clusterboot (B=200)
...
# D. MULTIPLE-SEED CONTROL SAMPLING ROBUSTNESS
#
# FIX: controls_s$cluster is now set to "0" (character), and
# data_bd$cluster is forced to character before bind_rows(),
# exactly mirroring the fix in the main script. This prevents
# the "Can't combine factor and double" error that occurred
# here in the previous draft.
############################################################
...
cat("\n============================================================\n")
cat("H. METHODS TEXT SNIPPET (copy into thesis Methods section)\n")
cat("============================================================\n")
...
# ------------------------------------------------------------
# Andrew explicitly named Consensus Clustering as a method to
...
############################################################
# VALIDATION SUMMARY — long format, explicit labels
# FIX: previous draft tried to cram vectors of different
# lengths (single Jaccard values, 5 ARI splits, 5 variable
# removal ARIs) into ONE Result column, which silently
# recycled values into the wrong rows. Each block is now
# built and labelled separately, then row-bound.
############################################################
...
cat("\n=== ANDREW FEEDBACK ADDITIONS COMPLETE ===\n")
